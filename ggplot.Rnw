...
% --------------------------------------------------------------
\begin{frame}[fragile]
\frametitle{Alternative modes to say the same thing}
...
\item Check the name of your variables Sepal.length \neq sepal.length \neq SepalLength
\item Be consistent with your variable naming. I like sepal_length
\item Have you closed all your parenthesis? This:
ggplot(aes(x = Sepal.Length, y = Sepal.Width) +
geom_point()
won´t work
\end{itemize}
...
What if you also want to see all the the individual points?
...
setwd('~/Dropbox/repos/ggplot2-lecture/')
@
...
% --------------------------------------------------------------
\section*{tidyr and dplyr are key for using \texttt{R}}
\frame{\sectionpage}

\begin{frame}[fragile]
\frametitle{tidyr and dplyr}
